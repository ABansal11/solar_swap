...
export default function NeighborhoodMap({
  ...
  useEffect(() => {
    ...
    setFlowLines(prev => [...prev, { fromX: fromHouse.x, fromY: fromHouse.y, toX: toHouse.x, toY: toHouse.y, id: lineId }]);
    setTimeout(() => { setFlowLines(prev => prev.filter(l => l.id !== lineId)); }, 1500);
  ...
  for (const p of participants) participantByHouse[p.houseId] = p;

  const iotHouseIds = new Set<number>();
  for (const e of pendingEscrows) {
    if (e.status === 'pending_iot' && e.houseId) iotHouseIds.add(e.houseId);
  }
    ...
    <div className="relative">
      ...
      >
        ...
        {houses.map(house => (
          <line
            key={`conn-${house.id}`}
            x1={house.x} y1={house.y} x2={CENTER.x} y2={CENTER.y}
        ...
        {houses.map(house => {
          ...
          return (
            <circle
              ...
              cx={house.x} cy={house.y} r="32"
              fill="none" stroke="#c4a035" strokeWidth="2.5" strokeDasharray="6,3"
        ...
        {flowLines.map(line => (
          <g key={line.id}>
            <line
              ...
              stroke="#c4a035" strokeWidth="2.5" strokeDasharray="6,4"
              markerEnd="url(#arrowhead)" opacity="0.9" filter="url(#glow)"
            >
        ...
        <g transform={`translate(${CENTER.x - 25}, ${CENTER.y - 45})`}>
          ...
          <rect
            x="3" y={10 + (70 - batteryFillHeight)} width="44" height={batteryFillHeight}
            rx="4" ry="4" fill={batteryColor} opacity="0.7" clipPath="url(#batteryClip)"
          />
...
function HouseIcon({
  house, cx, cy, participantName, iotPending = false,
}: {
  house: House; cx: number; cy: number; participantName?: string; iotPending?: boolean;
}) {
  ...
  return (
    <g transform={`translate(${cx - 22}, ${cy - 28})`} className="cursor-pointer" opacity={isOccupied || participantName === undefined ? 1 : 0.4}>
      ...
      {participantName !== undefined ? (
        isOccupied ? (
          <text x="22" y="58" textAnchor="middle" fill="#c4a035" fontSize="7" fontWeight="bold">
